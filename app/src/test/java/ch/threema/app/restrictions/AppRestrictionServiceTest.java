...
public class AppRestrictionServiceTest {
    ...
    @PrepareForTest({ThreemaApplication.class, Build.VERSION.class, WorkManagerImpl.class})
    ...
    public void testFilterNonWorkMdmParametersFromPreferences() throws Exception {
        ...
        ServiceManager serviceManagerMock = PowerMockito.mock(ServiceManager.class);
        when(serviceManagerMock.getPreferenceStore()).thenReturn(preferenceStoreMock);
        TaskCreator taskCreatorMock = new TaskCreator(serviceManagerMock);
        when(serviceManagerMock.getTaskCreator()).thenReturn(taskCreatorMock);
        when(serviceManagerMock.getMultiDeviceManager()).thenReturn(mock(MultiDeviceManager.class));
        when(serviceManagerMock.getTaskManager()).thenReturn(mock(TaskManager.class));
    ...
    private void mockStaticWorkManagerImpl() {
        WorkManagerImpl workManagerImplMock = mock(WorkManagerImpl.class);
        when(workManagerImplMock.getWorkInfosForUniqueWork("ApplyAppRestrictions")).thenReturn(new AbstractFuture<>() {
            @Override
            public List<WorkInfo> get() {
                return Collections.emptyList();
            }
        });
        PowerMockito.mockStatic(WorkManagerImpl.class);
        when(WorkManagerImpl.getInstance(any())).thenReturn(workManagerImplMock);
    }
